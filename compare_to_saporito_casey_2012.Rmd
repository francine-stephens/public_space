...
## Race and EVI correlations

2010 matches up with Saporito and Casey, who used 2010 ACS data. 
...
2000 correlations included for good measure. 
```{r correlation 2000}
##2000
...
Need to address change in GEOIDs for 1990 in order to bring in for comparisons. 

## Green Space Contact by Race

```{r calculate median EVI by race, message=F, warning=F}
...
```{r mean evi differences between race groups, warning=F, message=F}
...
## EVI DIFFERENCES
knitr::kable(green_exposure_race_diffs_by_place %>%
  ungroup() %>%
  select(-PLACEFP_DERIVED, -PLACE_DERIVED_NM) %>%
  summarize_all(mean, na.rm=T), 
  caption = "EVI Race Group Differences",
...
```{r run regression for bw differences, message=F, warning=F}
reg_bw_diff_evi <- lm(BLACK_WHITE_EVI_DIFF ~ dissim_bw, data=full_place_data)
summary(reg_bw_diff_evi ) 
