...
parks_per_places <- usa_parks_prj %>%
  st_join(., places %>% st_transform(albersea), join = st_within) %>% 
  mutate(
    within_area = st_area(.),
  ) %>%
  group_by(GEOID) %>%
  summarize(parks = n(),
            ...
            total_clip_area = sum(within_area)
            ) 
...
## intersection join
parks_per_places_intersection <- usa_parks_prj %>% 
...
```{r park intersections results}
parks_per_places_intersection %>%
