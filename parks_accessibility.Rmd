...
parks_per_places %>%
    select(parks, total_park_area, total_park_area_sqmi, NAME, STATEFP, GEOID) %>%
  arrange(-parks) %>%
  ...
  relocate(NAME, state, parks, total_park_area_sqmi, percent_of_land_area)
  # 54,083 parks 
...
parks_per_places %>%
  ...
  left_join(., places_area %>% select(GEOID, area_sqmi), by = "GEOID") %>%
  mutate(percent_of_land_area = (total_park_area_sqmi/area_sqmi) * 100) %>%
  relocate(NAME, state, total_park_area_sqmi, percent_of_land_area, parks) %>%
  arrange(percent_of_land_area) %>%
  filter(percent_of_land_area > 1 & percent_of_land_area < 100)
...
places %>% 
  filter(!GEOID %in% parks_per_places_intersection$GEOID) %>%
  st_set_geometry(NULL)
...
```{r parks per county}
parks_per_county %>% 
  select(parks, total_park_area, total_park_area2, NAME, STATEFP, GEOID) %>%
  arrange(-parks) %>%
  left_join(., state_identifiers, by = c("STATEFP" ="state_code")
            ) %>%
  left_join(., counties_area %>% select(GEOID, area_sqmi), by = "GEOID") %>%
  mutate(percent_of_land_area = (total_park_area_sqmi/area_sqmi) * 100) %>%
  relocate(NAME, state, parks, total_park_area_sqmi, percent_of_land_area)
